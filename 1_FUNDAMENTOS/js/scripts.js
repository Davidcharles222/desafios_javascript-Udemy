...
function isPrime(num) {
  if (num <= 1) {//se número for menor e igual a 1 retorne falso
    return false;
  }

  for (let i = 2; i * i <= num; i++) {
    //Ex. 2x2=4 <= num ? 'sim' >> i acrescenta 1
    //    3x3=9 <= num ? 'sim' >> i acrescenta 1
    //   4x4x=16 <= num? 'não'
    if (num % i === 0) {// se num dividido por i for igual a 0 retorne falso
      return false;
    }
  }

  return true;//caso contrário verdadeiro
}

for (let i = 2; i <= N; i++) {
  if (isPrime(i)) {//se a função for true, console.log os numeros primos
    console.log(i);
  }
}
